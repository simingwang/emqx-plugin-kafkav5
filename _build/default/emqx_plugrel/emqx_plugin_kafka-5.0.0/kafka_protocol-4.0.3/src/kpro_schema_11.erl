%% Generated from kafka 1.1, kept for backward compatibility.
%% `kpro_schema' module is generated from later versions.
%% There is no change is write format of the encoded requests and responses
%% only changes in field names.
-module(kpro_schema_11).
...
print_diff() ->
  APIs = all_apis(),
  [print_diff(API) || API <- APIs],
  [print_diff_fields(API) || API <- APIs],
  ok.

print_diff(API) ->
  {Min, MaxOld} = vsn_range(API),
  {Min, MaxNew} = kpro_schema:vsn_range(API),
  case MaxOld < MaxNew of
    true ->
      io:format(user, "      - ", []),
        case MaxOld+1 == MaxNew of
          true -> io:format(user, "`~p`: ~p\n", [API, MaxNew]);
          false -> io:format(user, "`~p`: ~p-~p\n", [API, MaxOld+1, MaxNew])
        end;
    false ->
      ok
  end.

print_diff_fields(API) ->
  {_, MaxOld} = vsn_range(API),
  ScOld = req(API, MaxOld),
  ScNew = kpro_schema:req(API, MaxOld),
  try
    case cmp(ScOld, ScNew) of
      [] -> ok;
      Diff ->
        io:format(user, "      - `~p`:\n", [API]),
        print_diff_fields(8, Diff)
    end
  catch
    throw : Reason ->
      error({API, Reason})
...
cmp([], []) -> [];
cmp([H | Old], [H | New]) -> cmp(Old, New);
cmp({array, Old}, {array, New}) ->
  {array, cmp(Old, New)};
cmp(TypeOld, TypeNew) when is_atom(TypeOld) ->
  {primitive_to_struct, {TypeOld, TypeNew}};
cmp(TypeOld, TypeNew) when is_atom(TypeNew) ->
  {struct_to_primitive, {TypeOld, TypeNew}};
cmp([{Name, TypeOld} | Old], [{Name, TypeNew} | New]) ->
  %% same name, different type
  case is_atom(TypeOld) andalso is_atom(TypeNew) of
    true -> throw(wtf);
    false -> ok
  end,
  [{Name, cmp(TypeOld, TypeNew)} | cmp(Old, New)];
cmp([{NameOld, Type} | Old], [{NameNew, Type} | New]) ->
  % same type, different name
  [{{NameOld, NameNew}, []} | cmp(Old, New)];
cmp([{NameOld, TypeOld} | Old], [{NameNew, TypeNew} | New]) ->
  %% different name, different type
  case is_atom(TypeOld) andalso is_atom(TypeNew) of
    true -> throw({NameOld, NameNew, TypeOld, TypeNew});
    false -> ok
  end,
  [{{NameOld, NameNew}, cmp(TypeOld, TypeNew)} | cmp(Old, New)].

print_diff_fields(_, []) -> ok;
print_diff_fields(Indent, {primitive_to_struct, {Old, New}}) ->
  indent(Indent),
  io:format(user, "- `~s` -> `~1000p`\n", [Old, New]);
print_diff_fields(Indent, {struct_to_primitive, {Old, New}}) ->
  indent(Indent),
  io:format(user, "- `~1000p` -> `~s`\n", [Old, New]);
print_diff_fields(Indent, [{Names, SubDiffs} | Diffs]) ->
  indent(Indent),
  case Names of
    {NameOld, NameNew} ->
      io:format("- `~s` -> `~s", [NameOld, NameNew]);
    Name ->
      io:format("- `~s", [Name])
  end,
  Deeper =
  ...
  io:format("\n"),
  print_diff_fields(Indent + 2, Deeper),
  print_diff_fields(Indent, Diffs).
